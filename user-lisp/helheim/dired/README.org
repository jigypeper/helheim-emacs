...
Dired local minor modes (such as ~dired-hide-details-mode~, ~dired-omit-mode~, etc.)
are converted into global minor modes, so their state persists across Dired
buffers.
...
Id Dired file deletion is done in 2 states: flag for deleteion, then execute
deletion.
...
| =S=        | Sorting menu                                                  |
| =K=        | Remove marked lines from Dired buffer without removing files. |
| =,u= or =gv= | Restore marks / deleted lines (~dired-undo~)                    |
| =zi= or =(=  | Toggle detailed information display.                          |
| =z.= or =)=  | Toggle hidden files display.                                  |

**** Filtering view ([[https://github.com/Fuco1/dired-hacks/blob/master/dired-filter.el][dired-filter]] from dired-hacks)
